...
void main(){
   ...
   /*Since the rubric needs to see the result of msg, this will run it correctly*/
   interrupt(33,3,"msg\0",buffer,&size);
   interrupt(33,0,buffer,0,0);
   interrupt(33,0,"\r\n\0",0,0);
   interrupt(33,3,"bigMSG\0",buffer,&size);
   interrupt(33,0,buffer,0,0);
   interrupt(33,0,"\r\n\0",0,0);

   /*This will run the executable fib file*/
...
/*Here was the original error function until the above was found available*/
/*void error(int bx){
  if(bx==0)
    printString("File not found");
  else printString("General DOS failure");
}*/
  ...
  for (i = 0; i < 16; i++){
    ...
    if (directorySector[32*i] != 0){
      ...
      for (j=0; j < 6; j++){
        ...
        if (directorySector[j+32*i] == 0 || fileName[j] == 0){
          break;
        }
        /*Check if characters match and continue checking remaining characters*/
        else if (directorySector[j+32*i] == fileName[j]){
          checkFound = 1;
        }
...
void readFile(char* fname, char* buffer, int* size){
  ...
  int i;
  int buffAddress = 0;
  /*Directory sector loads sector 2 to start loading the known flies*/
